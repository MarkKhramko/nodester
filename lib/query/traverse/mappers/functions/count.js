...
module.exports = function mapCOUNT(
	...
	try {
		...
		// If request to COUNT root model:
			...
			// Alias must match the response collection key for this model,
			// which the facade derives via outputNameForModel(). Deriving it
			// the same way here guarantees the count key is always
			// `<collection_key>_count` (e.g. "invoice_parses_count"), instead
			// of the smashed `.toLowerCase()` form ("invoiceparses_count").
			const outputName = outputNameForModel(rootModel);
			const countAttributeName = `${ outputName.plural }_count`;
			sequelizeQuery.attributes.push(
